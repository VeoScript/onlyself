...
import { useGetAllFiles } from '~/helpers/tanstack/queries/files-images/all';

interface FilesModalProps {
  username: string;
}

const FilesModal = ({ username }: FilesModalProps): JSX.Element => {
  ...
  const [isOpenDeleteFile, setIsOpenDeleteFile] = useState<boolean>(false);
  const [deleteType, setDeleteType] = useState<'delete-files' | 'delete-all-files'>('delete-files');
  const [fileType, setFileType] = useState<'FILE' | 'IMAGE'>('FILE');

  const { data: allFiles, isLoading: isLoadingAllFiles } = useGetAllFiles();
      ...
      >
        ...
        <div className="z-20 flex h-full w-full max-w-xl flex-col items-center overflow-hidden rounded-xl bg-white bg-opacity-20 font-poppins text-white backdrop-blur-sm">
            ...
            <div className="flex w-full flex-row items-center justify-between p-3">
              ...
              {filesImages && filesImages.pages[0].filesImages.length != 0 && (
                <button
                  ...
                  onClick={() => {
                    setDeleteType('delete-all-files');
          ...
          <div className="custom-scrollbar flex w-full flex-col items-start gap-y-1 px-3">
            {isLoadingFilesImages || isLoadingAllFiles ? (
                          ...
                          <div className="flex w-full flex-row items-center gap-x-2">
                            ...
                            <div className="flex max-w-sm flex-1 flex-col gap-y-1">
                              ...
                              {filesImage.is_anonymous ? (
                                <p className="text-xs font-light text-neutral-300">Anonymous</p>
                              ) : (
                                <Link
                          ...
                          <div className="flex flex-row items-center gap-x-2">
                            ...
                            >
                              <svg
                                ...
                                xmlns="http://www.w3.org/2000/svg"
                                aria-hidden="true"
                                className="h-4 w-4 text-white hover:text-blue-400"
                              >
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                            ...
                            <button
                              ...
                              onClick={() => {
                                setFileType(filesImage.type);
                                setDeleteType('delete-files');
                                setDeleteValue(filesImage.id);
                                setDeleteUrl(filesImage.delete_url);
                                setIsOpenDeleteFile(true);
                              }}
                            >
                              <svg
